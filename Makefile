...
IMAGE_TAG_BASE ?= ibm.com/ibm-iam-operator
...
$(CONTROLLER_GEN): $(LOCALBIN)
	...
	GOSUM=sum.golang.org GOBIN=$(LOCALBIN) go install -v -x sigs.k8s.io/controller-tools/cmd/controller-gen@$(CONTROLLER_TOOLS_VERSION)
...
## GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o build/_output/bin/ibm-iam-operator-amd64 ./bin/manager
...
build-image-s390x: $(CONFIG_DOCKER_TARGET) ## Build the Operator for Linux on s390x
	$(CONTAINER_CLI) run --rm --privileged docker.io/multiarch/qemu-user-static:register --reset
	$(CONTAINER_CLI) build --log-level debug ${IMAGE_BUILD_OPTS} --build-arg="TARGET_GOARCH=s390x" -t $(REGISTRY)/$(IMG)-s390x:$(VERSION) -f build/Dockerfile.s390x .
