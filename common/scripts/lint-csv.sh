...
if [[ $ARCH == "x86_64" && $OS == "Linux" ]]; then
    ...
    curl -L -o /tmp/jq https://github.com/stedolan/jq/releases/download/jq-1.6/jq-osx-amd64
    curl -L -o /tmp/yq https://github.com/mikefarah/yq/releases/download/3.3.0/yq_darwin_amd64
    chmod +x /tmp/jq /tmp/yq
else
    exit 0
fi

CSV_PATH=deploy/olm-catalog/ibm-iam-operator/${VERSION}/ibm-iam-operator.v${VERSION}.clusterserviceversion.yaml
