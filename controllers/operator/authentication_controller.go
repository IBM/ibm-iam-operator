...
func (r *AuthenticationReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	//watchNamespaceValue, err := ctrlcommon.GetWatchNamespace()
	//if err != nil {
	//	return fmt.Errorf("failed to set up Authentication controller: %w", err)
	//}

	//nsList := strings.Split(watchNamespaceValue, ",")

	//routeVerbs := []string{"get", "list", "watch", "create", "delete", "update", "patch"}
	//hasRoutesAccess, err := r.hasAPIAccessInNamespaces(context.Background(), nsList, routev1.GroupVersion, "route", routeVerbs)
	//if err != nil {
	//	return fmt.Errorf("failed to determine authorization for Routes: %w", err)
	//}

	//operandRequestVerbs := []string{"create", "get", "list", "patch", "watch", "update", "delete"}
	//hasOperandRequestAccess, err := r.hasAPIAccessInNamespaces(context.Background(), nsList, operatorv1alpha1.GroupVersion, "operandrequest", operandRequestVerbs)
	//if err != nil {
	//	return fmt.Errorf("failed to determine authorization for OperandRequest: %w", err)
	//}
