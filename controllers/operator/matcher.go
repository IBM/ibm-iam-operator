...
// or[T] returns a matcherFunc[T] that returns whether any of the supplied
// matchers found a match.
func or[T client.Object](matchers ...matcherFunc[T]) matcherFunc[T] {
	return func(observed T) (matches bool) {
		matches = false
		for _, isMatch := range matchers {
			if isMatch(observed) {
...
func updatesValuesWhen[T client.Object](matches matcherFunc[T], keys ...string) (fn func(T, T) bool) {
	return func(observed, updates T) bool {
		if matches(observed) {
			return updateFields[T](observed, updates, keys...)
		}
		return false
	}
}
