...
var _ = Describe("OperandRequest handling", func() {
	...
	Describe("servicesNamespaceHasMongoDBService", func() {
		var mongoDBService *corev1.Service
		BeforeEach(func() {
			...
			cb = *cb.WithRuntimeObjects(authCR, mongoDBService)
			cl = cb.Build()
			r = &AuthenticationReconciler{
				Client: cl,
			}
		})
		It("returns true when mongodb service exists in the services namespace", func() {
			hasService := r.servicesNamespaceHasMongoDBService(context.Background(), authCR)
			Expect(hasService).To(BeTrue())
		})
		It("returns false when mongodb service is not found in the services namespace", func() {
			err := r.Delete(context.Background(), mongoDBService)
			Expect(err).ToNot(HaveOccurred())
			hasService := r.servicesNamespaceHasMongoDBService(context.Background(), authCR)
			Expect(hasService).To(BeFalse())
		})
		It("returns false when mongodb service is not found in the services namespace due to an unrelated error", func() {
			err := r.Delete(context.Background(), mongoDBService)
			Expect(err).ToNot(HaveOccurred())
	...
	Describe("needToMigrateMongoDB", func() {
		var mongoDBService *corev1.Service
		BeforeEach(func() {
			mongoDBService = &corev1.Service{
				TypeMeta: metav1.TypeMeta{
		...
		It("returns true when mongodb service exists in the services namespace and Authentication has no migration annotation", func() {
		...
		It("returns false when mongodb service does not exist in the services namespace and Authentication has migration annotation", func() {
			authCR.Annotations = map[string]string{operatorv1alpha1.AnnotationAuthMigrationComplete: "true"}
			err := r.Update(context.Background(), authCR)
			Expect(err).ToNot(HaveOccurred())
			err = r.Delete(context.Background(), mongoDBService)
		...
		It("returns false when mongodb service does not exist in the services namespace and Authentication has no migration annotation", func() {
			err := r.Delete(context.Background(), mongoDBService)
			Expect(err).ToNot(HaveOccurred())
			needsToMigrate := r.needToMigrateMongoDB(context.Background(), authCR)
			Expect(needsToMigrate).To(BeFalse())
