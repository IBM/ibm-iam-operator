...
func (r *AuthenticationReconciler) createOrUpdateZenExtension(authCR *operatorv1alpha1.Authentication) subreconciler.Fn {
	return func(ctx context.Context) (result *ctrl.Result, err error) {
		...
		// The following is just a check to see whether creating a
		// ZenExtension will do anything. Presumably if the front door is
		// enabled, it should do something, but it isn't guaranteed unless
		// IM is aware of the Zen install itself. If the Zen Operator Deployment is
		// present in the same namespace as this Operator, and it includes
		// our watch namespace in its watch list, then it should handle the
		// ZenExtension.
		...
		if result, err = r.getOrCreateZenExtension(ctx, unstrObserved, unstrDesired); subreconciler.ShouldRequeue(result, err) {
			return subreconciler.RequeueWithDelay(defaultLowerWait)
		}
