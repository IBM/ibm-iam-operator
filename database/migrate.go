...
func PlanMigrations(ctx context.Context, to, from dbconn.DBConn) (mq *migration.MigrationQueue, err error) {
	...
	*mq = make(migration.MigrationQueue, 0)
	...
	if _, ok := changelogs[v1schema.InitSchemas.Name]; !ok {
		reqLogger.Info("Adding migration", "migrationName", v1schema.InitSchemas.Name)
		*mq = append(*mq, migration.FromMigration(v1schema.InitSchemas).
			To(to).
			Build())
	}

	if _, ok := changelogs[v1schema.IncreaseOIDCUsernameSize.Name]; !ok {
	...
	if _, ok := changelogs[v1schema.MongoToV1.Name]; !ok && mongo != nil {
		reqLogger.Info("Adding migration", "migrationName", v1schema.MongoToV1.Name)
		*mq = append(*mq, migration.FromMigration(v1schema.MongoToV1).
			To(to).
			From(from).
			Build())
	}
