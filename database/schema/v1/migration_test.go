...
var _ = Describe("MigrationQueue", func() {
	DescribeTable("UpdatePrioritiesByDependencyCount",
		...
		Entry("InitSchemas alone",
			func() []*migration.Migration {
				return []*migration.Migration{
					InitSchemas,
				}
			}, []string{InitSchemas.Name}),
		Entry("InitSchemas and IncreaseOIDCUsernameSize",
			func() []*migration.Migration {
				return []*migration.Migration{
					IncreaseOIDCUsernameSize,
					InitSchemas,
				}
			}, []string{InitSchemas.Name, IncreaseOIDCUsernameSize.Name}),
		Entry("InitSchemas, IncreaseOIDCUsernameSize, CreateMetadataSchema, and MongoToV1",
			func() []*migration.Migration {
				return []*migration.Migration{
					IncreaseOIDCUsernameSize,
					MongoToV1,
			...
			}, []string{CreateMetadataSchema.Name, InitSchemas.Name, IncreaseOIDCUsernameSize.Name, MongoToV1.Name}),
	)
})
