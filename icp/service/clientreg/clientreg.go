 ...
 *******************************************************************************/
 ...
 func CreateClientCredentials(oidcreg *securityv1.Client, recorder record.EventRecorder) (*ClientCredentials, error) {
	 authServiceUrl := os.Getenv("IAM_AUTH_SERVICE_URL")
	 url := strings.Join([]string{authServiceUrl, "/oidc/endpoint/OP/registration"}, "")
	 serviceName := oidcreg.Name
	 clientCred := generateClientCredentials(serviceName)
 
	 payload := generatePayload(oidcreg, clientCred)
	 response, err := invokeRegistration(PostType, url, payload)
	 if response != nil && response.Status == "201 Created" {
		 oidcreg.Spec.ClientId = clientCred.CLIENT_ID
		 defer response.Body.Close()
		 return clientCred, nil
	 } else {
		 handleOIDCClientError(oidcreg, response, err, PostType, recorder)
 ...
 func UpdateClientCredentials(oidcreg *securityv1.Client, secret *corev1.Secret, recorder record.EventRecorder) (*ClientCredentials, error) {
	 authServiceUrl := os.Getenv("IAM_AUTH_SERVICE_URL")
	 ...
	 response, err := invokeRegistration(PutType, url, payload)
	 if response != nil && response.Status == "200 OK" {
		 defer response.Body.Close()
		 return clientCred, nil
	 } else {
		 handleOIDCClientError(oidcreg, response, err, PutType, recorder)
		 err = fmt.Errorf("Error occurred during update oidc client regstration")
		 return nil, err
	 }
 }
 
 func DeleteClientCredentials(client *securityv1.Client, recorder record.EventRecorder) error {
	 clientId := client.Spec.ClientId
	 if clientId != "" {
		 authServiceUrl := os.Getenv("IAM_AUTH_SERVICE_URL")
		 url := strings.Join([]string{authServiceUrl, "/oidc/endpoint/OP/registration/", clientId}, "")
		 response, err := invokeRegistration(DeleteType, url, "")
 ...
 func invokeRegistration(requestType string, requestUrl string, payload string) (*http.Response, error) {
	 oauthAdmin := os.Getenv("OAUTH_ADMIN")
	 clientRegistrationSecret := os.Getenv("OAUTH2_CLIENT_REGISTRATION_SECRET")
	 ...
	 if resp != nil && resp.StatusCode >= 400 {
		 ...
		 log.Printf("Invoke registration has failed: ", string(errorDetails))
 ...
 func getClientCredentials(oidcreg *securityv1.Client, recorder record.EventRecorder) (*http.Response, error) {
	 clientId := oidcreg.Spec.ClientId
	 authServiceUrl := os.Getenv("IAM_AUTH_SERVICE_URL")
	 url := strings.Join([]string{authServiceUrl, "/oidc/endpoint/OP/registration/", clientId}, "")
	 response, err := invokeRegistration(GetType, url, "")
	 if response != nil && response.Status == "200 OK" {
		 return response, nil
	 } else {
 ...
 func ClientIdExists(client *securityv1.Client, recorder record.EventRecorder) (bool, *ClientCredentials, error) {
	 clientId := client.Spec.ClientId
	 if clientId != "" {
		 resp, err := getClientCredentials(client, recorder)
		 if err != nil {
			 return false, nil, err
		 } else if clientId != "" && resp.Status == "200 OK" {
			 clientCreds, errRes := getCredentialsFromResponse(resp)
			 if errRes != nil {
 ...
 func getCredentialsFromResponse(response *http.Response) (*ClientCredentials, error) {
 ...
 func generateClientCredentials(serviceName string) *ClientCredentials {
	 log.Printf("OidcClient-Watcher, Generate ClientID & Secret")
	 rule := `^([a-z0-9]){32,}$`
 ...
 func generatePayload(oidcreg *securityv1.Client, clientCred *ClientCredentials) string {
 ...
 func GetZenInstance(client *securityv1.Client) (*ZenInstance, error) {
 ...
 func DeleteZenInstance(client *securityv1.Client) error {
 ...
 func CreateZenInstance(client *securityv1.Client) error {
	 payloadJSON := map[string]interface{}{
		 "clientId":       client.Spec.ClientId,
