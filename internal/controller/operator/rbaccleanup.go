...
func (r *AuthenticationReconciler) cleanupOldRBAC(ctx context.Context, req ctrl.Request) (result *ctrl.Result, err error) {
	...
	canDeleteCRB, err := r.canDeleteClusterRoleBinding(debugCtx)
	if err != nil {
	...
	canDeleteCR, err := r.canDeleteClusterRole(debugCtx)
	if err != nil {
...
// canDeleteClusterRoleBinding checks if the operator has permission to delete ClusterRoleBindings
func (r *AuthenticationReconciler) canDeleteClusterRoleBinding(ctx context.Context) (bool, error) {
	log := logf.FromContext(ctx)
	...
	err := r.Client.Create(ctx, ssar)
	if err != nil {
		log.Error(err, "Failed to create SelfSubjectAccessReview for ClusterRoleBinding")
		return false, err
	}

	if ssar.Status.Allowed {
		log.V(1).Info("Operator has permission to delete ClusterRoleBindings")
		return true, nil
	}

	log.Info("Operator does not have permission to delete ClusterRoleBindings", "reason", ssar.Status.Reason)
	return false, nil
}

// canDeleteClusterRole checks if the operator has permission to delete ClusterRoles
func (r *AuthenticationReconciler) canDeleteClusterRole(ctx context.Context) (bool, error) {
	log := logf.FromContext(ctx)
	...
	err := r.Client.Create(ctx, ssar)
	if err != nil {
		log.Error(err, "Failed to create SelfSubjectAccessReview for ClusterRole")
		return false, err
	}

	if ssar.Status.Allowed {
		log.V(1).Info("Operator has permission to delete ClusterRoles")
		return true, nil
	}

	log.Info("Operator does not have permission to delete ClusterRoles", "reason", ssar.Status.Reason)
	return false, nil
}
