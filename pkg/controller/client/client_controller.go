...
import (
	...
	clientreg "github.com/IBM/ibm-iam-operator/icp/service/clientreg"
	condition "github.com/IBM/ibm-iam-operator/pkg/api/util"
	oidcv1 "github.com/IBM/ibm-iam-operator/pkg/apis/operator/v1alpha1"
...
func (r *ReconcileClient) Reconcile(contect context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	errorRg := r.processOidcRegistration(reqLogger, instance, r.recorder)
	if errorRg != nil {
...
func (r *ReconcileClient) processOidcRegistration(reqLogger logr.Logger, client *oidcv1.Client, evtRecorder record.EventRecorder) error {
	...
	errReg, isDeleteEvent = r.processDeleteRegistration(reqLogger, client, evtRecorder)
	if isDeleteEvent == true && errReg == nil {
		...
		clientCreds := &clientreg.ClientCredentials{}
		secret := &corev1.Secret{}
		clientIdExists, clientCreds, errReg = clientreg.ClientIdExists(client, evtRecorder)
		if errReg != nil {
			...
			clientCreds, errReg = clientreg.CreateClientCredentials(client, evtRecorder)
			if errReg != nil {
				return errReg
			}
			secret, errSecret = r.newSecretForClient(client, clientCreds)
			if isRoksEnabled == "true" {
				_, errOauthClient = r.newOAuthClientForClient(client, clientCreds)
			}
			...
			secret = r.reconcileSecret(client, clientCreds)
			if isRoksEnabled == "true" {
				_ = r.reconcileOAuthClient(client, clientCreds)
			}
			clientCreds, errReg = clientreg.UpdateClientCredentials(client, secret, evtRecorder)
		...
		if errReg == nil && errSecret == nil && errOauthClient == nil && errZen == nil {
			condition.SetClientCondition(client,
				...
				clientreg.ReasonCreateClientSuccessful,
				clientreg.MessageClientSuccessful)
			if clientIdExists == false {
				r.recorder.Event(client, corev1.EventTypeNormal, clientreg.ReasonCreateClientSuccessful, clientreg.MessageCreateClientSuccessful)
			} else {
				r.recorder.Event(client, corev1.EventTypeNormal, clientreg.ReasonUpdateClientSuccessful, clientreg.MessageUpdateClientSuccessful)
			}
		}
	}
	...
	errUpdate := r.client.Update(context.TODO(), client)
	if errUpdate != nil {
		return errUpdate
	}
	...
	errUpdate = r.client.Status().Update(context.TODO(), client)
	if errUpdate != nil {
		return errUpdate
	}
	if errReg != nil || errSecret != nil || errOauthClient != nil || errZen != nil {
...
func (r *ReconcileClient) processZenRegistration(reqLogger logr.Logger, client *oidcv1.Client, evtRecorder record.EventRecorder) error {
	...
	zenReg, zenErr := clientreg.GetZenInstance(client)
	if zenErr != nil {
		//Set the status to false since zen registration cannot be completed
		condition.SetClientCondition(client, oidcv1.ClientConditionReady, oidcv1.ConditionFalse, clientreg.ReasonCreateZenRegistrationFailed,
			clientreg.MessageCreateZenRegistrationFailed)
		evtRecorder.Event(client, corev1.EventTypeWarning, clientreg.ReasonCreateZenRegistrationFailed, zenErr.Error())
		return zenErr
	}
	...
	err := clientreg.CreateZenInstance(client)
	if err != nil {
		//Set the status to false since zen registration cannot be completed
		condition.SetClientCondition(client, oidcv1.ClientConditionReady, oidcv1.ConditionFalse, clientreg.ReasonCreateZenRegistrationFailed,
			clientreg.MessageCreateZenRegistrationFailed)
		evtRecorder.Event(client, corev1.EventTypeWarning, clientreg.ReasonCreateZenRegistrationFailed, err.Error())
		return err
	}
...
func (r *ReconcileClient) processDeleteRegistration(reqLogger logr.Logger, client *oidcv1.Client, evtRecorder record.EventRecorder) (error, bool) {

	isInstanceMarkedToBeDeleted := client.GetDeletionTimestamp() != nil
	// Update finalizer to allow delete CR
	if isInstanceMarkedToBeDeleted {
		errDel := clientreg.DeleteClientCredentials(client, evtRecorder)
		if errDel != nil {
		...
		if isRoksEnabled == "true" {
			...
			errGet := r.Reader.Get(context.TODO(), types.NamespacedName{Name: clientId}, oAuthClientToBeDeleted)
			if errGet == nil {
				errDelOAuthClient := r.client.Delete(context.TODO(), oAuthClientToBeDeleted)
		...
		if client.Spec.ZenInstanceId != "" {
			err := clientreg.DeleteZenInstance(client)
			if err != nil {
...
func (r *ReconcileClient) reconcileSecret(client *oidcv1.Client, clientCreds *clientreg.ClientCredentials) *corev1.Secret {
	found := &corev1.Secret{}
	err := r.Reader.Get(context.TODO(), types.NamespacedName{Name: client.Spec.Secret, Namespace: client.Namespace}, found)
	if err == nil {
		return found
	} else {
		secret := &corev1.Secret{}
		secret, errSec := r.newSecretForClient(client, clientCreds)
		if errSec == nil {
			return secret
		} else {
			return nil
...
func (r *ReconcileClient) reconcileOAuthClient(client *oidcv1.Client, clientCreds *clientreg.ClientCredentials) *oauthv1.OAuthClient {
	found := &oauthv1.OAuthClient{}

	err := r.Reader.Get(context.TODO(), types.NamespacedName{Name: clientCreds.CLIENT_ID}, found)
	if err != nil {
		return nil
	}

	oauthclient, errOauth := r.updateOAuthClientForClient(found, client, clientCreds)
	if errOauth == nil {
		return oauthclient
	} else {
		return nil
...
func (r *ReconcileClient) newSecretForClient(client *oidcv1.Client, clientCreds *clientreg.ClientCredentials) (*corev1.Secret, error) {
	...
	if clientCreds != new(clientreg.ClientCredentials) && clientCreds != nil {
		if secret.Data == nil {
...
// newPodForCR returns a busybox pod with the same name/namespace as the cr
func (r *ReconcileClient) newOAuthClientForClient(client *oidcv1.Client, clientCreds *clientreg.ClientCredentials) (*oauthv1.OAuthClient, error) {
	reqLogger := log.WithValues("Request.Namespace", client.Namespace, "client.Name", client.Name)
...
// newPodForCR returns a busybox pod with the same name/namespace as the cr
func (r *ReconcileClient) updateOAuthClientForClient(oauthclient *oauthv1.OAuthClient, client *oidcv1.Client, clientCreds *clientreg.ClientCredentials) (*oauthv1.OAuthClient, error) {
	reqLogger := log.WithValues("Request.Namespace", client.Namespace, "client.Name", client.Name)
	...
	errUpdate := r.client.Update(context.TODO(), oauthclient)
	if errUpdate != nil {
