...
func (r *ReconcileSecurityOnboarding) Reconcile(context context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		...
		r.event.Eventf(instance, "Warning", "Onboarding Job failed", "Job dependencies not met, please check troubleshooting document %s for reasons and solutions", recResult)
		return recResult, err
	}
...
func (r *ReconcileSecurityOnboarding) handleJob(instance *operatorv1alpha1.SecurityOnboarding) (reconcile.Result, error) {
	...
	if !secJobExists {
		...
		reqLogger.Info("Restart required for securityOnboardJob", "Job.Namespace", instance.Namespace, "Job.Name", "security-onboarding")
